...
-- Kỹ thuật (2)
('kythuat01',    'kt01@123',     1),
('kythuat02',    'kt02@123',     1),

-- Nhân viên (5)
('nhanvien01',   'nv01@123',     1),
('nhanvien02',   'nv02@123',     1),
('nhanvien03',   'nv03@123',     1),
('nhanvien04',   'nv04@123',     1),
('nhanvien05',   'nv05@123',     1),

-- Khách hàng có tài khoản (5)
('khach01',      'kh01@123',     1),
('khach02',      'kh02@123',     1),
('khach03',      'kh03@123',     1),
('khach04',      'kh04@123',     1),
...
-- Khách vãng lai (5)
('guest01',      'guest01@123',  0),
('guest02',      'guest02@123',  0),
('guest03',      'guest03@123',  0),
('guest04',      'guest04@123',  0),
('guest05',      'guest05@123',  0);
GO
...
(MaTaiKhoan, HoTen, GioiTinh, NgaySinh, CCCD, SoDienThoai, DiaChi, ChucVu, TrangThaiLamViec)
VALUES
-- Quản lý (2)
...
(MaTaiKhoan, SoDienThoai, HoTen, CCCD, DiaChi, BienSoXeMacDinh)
VALUES
-- Khách hàng có tài khoản (5)
...
(MaNhanVien, ThoiGianNhanCa, ThoiGianGiaoCa, TienDauCa, TongTienHeThong, TienMatBanGiao, GhiChuBanGiao, TrangThaiCa)
VALUES
-- Ca đã chốt
...
------------------------------------------------------------
INSERT INTO CauHinhGia (TenCauHinh, MaLoaiXe, GioBatDau, GioKetThuc)
VALUES (N'Giá Ô tô 4 chỗ Ngày', 2, '06:00:00', '18:00:00');
DECLARE @Oto4Ngay INT = SCOPE_IDENTITY();

-- Block 1: 2 tiếng đầu
INSERT INTO ChiTietGia VALUES
(@Oto4Ngay, 1, 120, 20000, 0);

-- Block 2: mỗi 1 tiếng sau
INSERT INTO ChiTietGia VALUES
(@Oto4Ngay, 2, 60, 10000, 1);
GO
...
------------------------------------------------------------
INSERT INTO CauHinhGia (TenCauHinh, MaLoaiXe, GioBatDau, GioKetThuc)
VALUES (N'Giá Ô tô 7 chỗ Ngày', 3, '06:00:00', '18:00:00');
DECLARE @Oto7Ngay INT = SCOPE_IDENTITY();

INSERT INTO ChiTietGia VALUES
(@Oto7Ngay, 1, 120, 30000, 0),
(@Oto7Ngay, 2, 60, 15000, 1);
GO
...
------------------------------------------------------------
INSERT INTO CauHinhGia (TenCauHinh, MaLoaiXe, GioBatDau, GioKetThuc)
VALUES (N'Giá Xe tải Ngày', 4, '06:00:00', '18:00:00');
DECLARE @XeTaiNgay INT = SCOPE_IDENTITY();

INSERT INTO ChiTietGia VALUES
(@XeTaiNgay, 1, 120, 40000, 0),
(@XeTaiNgay, 2, 60, 20000, 1);
GO

------------------------------------------------------------
-- KIỂM TRA
------------------------------------------------------------
SELECT chg.TenCauHinh, ctg.*
FROM ChiTietGia ctg
JOIN CauHinhGia chg ON ctg.MaCauHinh = chg.MaCauHinh
ORDER BY chg.MaCauHinh, ctg.ThuTuBlock;

-- KIỂM TRA
SELECT * FROM ChiTietGia;
...
/* =========================
   1. THẺ XE
   LoaiThe: 0 = Vãng lai, 1 = Thẻ tháng
...
/* =========================
   3. LỊCH SỬ GIA HẠN VÉ
   ========================= */
INSERT INTO LichSuGiaHanVe (
    MaVeThang,
    NgayGiaHan,
    ThoiHanCu,
    ThoiHanMoi,
...
--SELECT MaCa, MaNhanVien, TrangThaiCa
--FROM CaLamViec;

--SELECT MaViTri, TenViTri, TrangThai
--FROM ViTriDo
--ORDER BY MaViTri;
...
VALUES
-- Xe vãng lai đang trong bãi
('THE-XM-001', NULL, 7, DATEADD(MINUTE, -30, GETDATE()),
 '30F-123.45', NULL, 4,
 NULL, NULL, NULL, NULL,
 0, 0),

-- Xe có đặt chỗ, đã ra
('THE-OTO4-001', 1, 7, DATEADD(HOUR, -3, GETDATE()),
 '29A-456.78', NULL, 6,
 8, DATEADD(HOUR, -1, GETDATE()), '29A-456.78', NULL,
 30000, 1),

-- Xe thẻ tháng
('THE-XM-002', NULL, 8, DATEADD(HOUR, -5, GETDATE()),
 '99B-888.99', NULL, 8,
 9, DATEADD(HOUR, -2, GETDATE()), '99B-888.99', NULL,
 0, 1);
GO
...
VALUES
-- Xe mất thẻ
(GETDATE(), 1, N'Xe mất thẻ', 'THE-XM-001', 1,
 N'Khách làm mất thẻ xe máy khi ra cổng', 0),

(GETDATE(), 2, N'Xe mất thẻ', 'THE-OTO4-001', 4,
 N'Khách không tìm thấy thẻ gửi xe ô tô', 1),

-- Lỗi barrier
(GETDATE(), 3, N'Lỗi barrier', NULL, NULL,
 N'Barrier không tự động mở khi quét thẻ', 0),

(GETDATE(), 1, N'Lỗi barrier', NULL, NULL,
 N'Barrier bị kẹt, cần bảo trì gấp', 2),

-- Lỗi camera
(GETDATE(), 2, N'Lỗi camera', NULL, 7,
 N'Camera khu C không ghi nhận được biển số', 1),

(GETDATE(), 3, N'Lỗi camera', NULL, 10,
 N'Camera mờ, hình ảnh không rõ vào ban đêm', 2),

-- Tranh chấp phí
(GETDATE(), 1, N'Tranh chấp phí', 'THE-XM-002', 3,
 N'Khách không đồng ý mức phí gửi xe', 0),

(GETDATE(), 2, N'Tranh chấp phí', 'THE-OTO7-001', 8,
 N'Khách phản ánh bị tính sai giờ gửi', 1),

-- Khẩn cấp
(GETDATE(), 3, N'Khẩn cấp', NULL, 5,
 N'Xe bị chảy dầu nghiêm trọng trong bãi', 0),

-- Khác
(GETDATE(), 1, N'Khác', NULL, NULL,
 N'Sự cố phát sinh ngoài giờ cao điểm', 2);
GO
